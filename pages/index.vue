<template>
  ...
  <ProductList :data="data?.data.courses!" title="最新课程" />
  <ProductList :data="data?.data.columns!" title="最新专栏" type="column" />
</template>

<script setup lang="ts">
  useHead({ title: 'iCourse' });
  ...
  if (process.server && error.value) showError('获取数据失败');
</script>
